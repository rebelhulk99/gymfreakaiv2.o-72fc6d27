...
export function CameraInterface({ workout, username, onBack }: CameraInterfaceProps) {
  ...
  }, [isActive, isPaused, workout.name]);
  ...
  // NEW: call FastAPI backend with current frame
  const testBackend = async () => {
    ...
    // IMPORTANT: these names must match your FastAPI endpoint
    // Adjust the URL and field names if your Swagger docs differ.
    formData.append('exercise', workout.name);      // or a mapped exercise id
    formData.append('file', blob, 'frame.jpg');    // name must match UploadFile field

    try {
      const res = await fetch('/api/count-reps', {
        method: 'POST',
        body: formData,
      });
      ...
      const score =
        typeof data.form_score === 'number'
          ? data.form_score
          : formAccuracy / 100;

      setReps(newReps);
      setFormAccuracy(Math.max(0, Math.min(100, Math.round(score * 100))));
      // You can also use data.is_valid and data.feedback later.
    } catch (err) {
      ...
      <div className="flex-1 flex gap-6 p-6 max-w-7xl mx-auto w-full">
        <div className="flex-1 relative bg-gym-dark rounded-lg overflow-hidden border-2 border-gym-gold/30">
          ...
          <div className="absolute bottom-4 left-4 right-4 flex gap-2">
            ...
            {/* NEW: test FastAPI backend */}
            <button
              onClick={testBackend}
              className="flex-1 bg-gym-gold/20 border border-gym-gold text-gym-gold py-2 rounded-lg hover:bg-gym-gold hover:text-gym-black transition-all duration-300 font-semibold"
            >
              TEST AI
            </button>
          </div>
